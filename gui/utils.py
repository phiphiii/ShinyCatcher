...
MAX_WHITE_PIXELS_THRESHOLD = 30000
...
def detect_popup(frame):
    ...
    is_popup = WHITE_PIXELS_THRESHOLD < white_pixels < MAX_WHITE_PIXELS_THRESHOLD

    return is_popup, white_pixels
...
def display_frame(frame, mask=None, ui_kwargs=None):
    """Routes the frame to GUI or standard OpenCV window, carrying UI stats."""
    if FRAME_CALLBACK:
        # Pass the dictionary with stats to the GUI callback
        FRAME_CALLBACK(frame, mask, ui_kwargs)
    else:
        resized = cv2.resize(frame, (0, 0), fx=0.5, fy=0.5)
        cv2.imshow('Nintendo Stream', resized)
        if mask is not None:
            cv2.imshow('Yellow Mask', cv2.resize(mask, (0, 0), fx=1.0, fy=1.0))
        cv2.waitKey(10)


def active_sleep(seconds, capture_device, current_frame, state_name="WAITING", box_color=(0, 0, 255),
                 break_on_black=False, mode="NONE"):
    """Pauses execution while constantly updating the video feed and GUI stats."""
    ...
    while time.time() - start_time < seconds:
        ...
        # Package current state and timer to send to GUI
        ui_data = {
            'status': state_name,
            'wait_time': f"{time.time() - start_time:.1f}s / {seconds:.1f}s"
        }

        display_frame(disp_frame, ui_kwargs=ui_data)
        if STOP_FLAG:
            break

    return latest_frame

PARTY_BOTTOM_ROI_X = 1350
PARTY_BOTTOM_ROI_Y = 800
PARTY_BOTTOM_ROI_W = 400
PARTY_BOTTOM_ROI_H = 200


def detect_party_bottom_cursor(frame):
    """
    Detects if the cursor is on the bottom right Pokémon in the party menu
    by looking for the bright white/gray of the open pokeball background.
    """
    roi = frame[PARTY_BOTTOM_ROI_Y:PARTY_BOTTOM_ROI_Y + PARTY_BOTTOM_ROI_H,
    PARTY_BOTTOM_ROI_X:PARTY_BOTTOM_ROI_X + PARTY_BOTTOM_ROI_W]

    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)

    # Look for bright white pixels
    lower_white = np.array([0, 0, 200])
    ...
    # If a large amount of white pixels is detected, the cursor is there
    return white_pixels > 4000