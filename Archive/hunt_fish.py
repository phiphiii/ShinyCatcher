...
def run(capture_device, esp, encounters, shiny_found):
    ...
    while True:
        ...
        if state == "CASTING_ROD":
            ...
            esp.write(b'A')
            current_frame = utils.active_sleep(1, capture_device, current_frame, "BAG_OPEN", roi_color, mode="FISH")
            esp.write(b'A')
            current_frame = utils.active_sleep(1, capture_device, current_frame, "SELECT_ROD", roi_color, mode="FISH")
            esp.write(b'A')
            ...
            current_frame = utils.active_sleep(2.8, capture_device, current_frame, "THROWING_POKEMON", roi_color, mode="FISH")
            roi_color = (255, 100, 0)
            if esp and not utils.TEST_MODE:
